...
核心职责：
1. 初始化时建立 FxDatabase，推导 SchemaSpec，ensure_table（幂等）。
...
4. total_count()：委托给 FxDatabase.count()。
"""
...
class SQLiteSink(Sink):
    ...
    def write(
        ...
        *,
        ...
        2. 批量调用 FxDatabase.upsert()，返回 (inserted, updated)。
        3. backup=True 时调用 FxDatabase.backup()（轮级备份时可关闭此开关）。
        4. 构建并返回 WriteResult。
    ...
    @property
    def db(self) -> FxDatabase:
        """暴露底层 FxDatabase，供高级用法（如 query / readonly_connect）。"""
        return self._db
