...
class HarvexError(Exception):
    """框架所有异常的基类。"""


class ConfigError(HarvexError):
    """配置缺失或非法（如必填项未填、schedule 表达式错误）。"""


class FetchError(HarvexError):
    """抓取阶段失败（网络、超时、对端 5xx、鉴权失败等）。"""


class ParseError(HarvexError):
    """解析阶段失败（页面结构变化、JSON 字段缺失等）。"""


class RecordValidationError(HarvexError):
